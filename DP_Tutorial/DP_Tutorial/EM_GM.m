...
[n,d] = size(X);
W = zeros(1,k); % mixing proportions
M = zeros(d,k); % component means
V = zeros(d,d,k);% component variances
...
clf, hold on
if d==1,
    c=30; % arbitrary constant to scale probability to fit histogram counts on a single figure
    hist(X,100);
    Q = zeros(size(R));
    for i=1:k,
        P = c*W(i)*normpdf(R,M(:,i),sqrt(V(:,:,i)));
        Q = Q + P;
        plot(R,P,'r-'); grid on, % plot individual component
    end
    plot(R,Q,'k-'); % plot mixture
    xlabel('X');
