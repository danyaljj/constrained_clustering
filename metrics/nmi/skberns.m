function [pw, ci, ll, ll_ave] = skberns(dtm, kc, varargin)

% SKBERNS clusters a group of documents, given by the document-term matrix
% (dtm), using the Bernoulli model-based stochastic k-means algorithm.
% This algorithm differs from KBERNS only in that the assignments of documents
% to clusters (for model parameter estimation) are sampled according to the
% posterior probability distribution.
%
% [pw, ci, ll, ll_ave] = skberns(dtm, kc, [[argID,] value, ...])
%
% Examples:
%	[pw,ci,ll,la] = skberns(dtm,kc)
%	[pw,ci,ll,la] = skberns(dtm,kc,'ml')
%	[pw,ci] = skberns(dtm,kc,'maxi',1,'iprobs',pw0,'training','ml')
%	[pw,ci] = skberns(dtm,kc,'maxi',10,'relerr',1e-3,'ipart',ci0)
%
% INPUT:
...
%
% See also: KBERNS, MIXBERNS, DABERNS, SKMNS, SKVMFS
