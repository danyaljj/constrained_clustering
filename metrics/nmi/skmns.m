function [pw, ci, ll, ll_ave] = skmns(dtm, kc, varargin)

% SKMNS clusters a group of documents, given by the document-term matrix
% (dtm), using the multinomial model-based stochastic k-means algorithm.
...
% This algorithm differs from KMNS only in that the assignments of documents
% to clusters (for model parameter estimation) are sampled according to the
% posterior probability distribution.
%
% [pw, ci, ll, ll_ave] = skmns(dtm, kc, [[argID,] value, ...])
%
% Examples:
%	[pw,ci,ll,la] = skmns(dtm,kc)
%	[pw,ci,ll,la] = skmns(dtm,kc,'ml')
%	[pw,ci] = skmns(dtm,kc,'maxi',1,'iprobs',pw0,'map')
%	[pw,ci] = skmns(dtm,kc,'maxi',10,'relerr',1e-3,'ipart',ci0)
%
% INPUT:
...
%
% See also: KMNS, MIXMNS, DAMNS, KBERNS, KVMFS
