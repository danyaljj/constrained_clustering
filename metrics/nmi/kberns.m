function [pw, ci, ll, ll_ave] = kberns(dtm, kc, varargin)

% KBERNS clusters a group of documents, given by the document-term matrix
% (dtm), using the multivariate Bernoulli model-based k-means algorithm. Each
...
%
% [pw, ci, ll, ll_ave] = kberns(dtm, kc, [[argID,] value, ...])
%
% Examples:
%	[pw,ci,ll,la] = kberns(dtm,kc)
%	[pw,ci] = kberns(dtm,kc,'maxi',1,'iprobs',pw0,'training','ml')
%	[pw,ci] = kberns(dtm,kc,'maxi',10,'relerr',1e-3,'ipart',ci0)
%
% INPUT:
...
%	                     each entry must be an integer within [1,kc] 
%	'iprobs'    (matrix) initial model parameters, size kc by nw,
%	                     Cannot be used simultaneously with 'ipart'
%	'training'  (string) parameter training algorithm, 'map' or
%	                     'ml' (default=map), which can be given without
%	                     preceeding argID 'training' (given the sparsity
%	                     and high dimensionality of document data, 'map'
...
%	ll_ave (vector) a trace of log-likelihood objective values,
%	                note that the objective of map learning is different
%	                from that of ml learning (with addition of log(prior))
%
% References:
...
%
% This function can also be used to do classification in two steps
% (suppose we train on dtm0, test on dtm1):
%	pw0 = kberns(dtm0,kc,'maxi',0,'ipart',classid0);
%	[pw1,ci] = kberns(dtm1,kc,'maxi',1,'iprobs',pw0);
% One then compares the resulting ci with the known class labels classid1
% to get the number of misclassified documents.
%
% See also: SKBERNS, BKBERNS, MIXBERNS, DABERNS, KMNS, KVMFS
