function [pw, ci, ll, ll_ave] = bkmns(dtm, kc, varargin)

% BKMNS clusters a group of documents, given by the document-term matrix
% (dtm), using the multinomial model-based balanced k-means algorithm.
...
%
% [pw, ci, ll, ll_ave] = bkmns(dtm, kc, [[argID,] value, ...])
%
% Examples:
%	[pw,ci,ll,la] = bkmns(dtm,kc)
%	[pw,ci,ll,la] = bkmns(dtm,kc,'minn',10)
%	[pw,ci] = bkmns(dtm,kc,'maxi',1,'iprobs',pw0,'map')
%	[pw,ci] = bkmns(dtm,kc,'maxi',10,'relerr',1e-3,'ipart',ci0)
%
% INPUT:
