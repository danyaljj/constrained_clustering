...
def print_top_words_for_topics(topics, tau, counts=None, n_words=10):
    ...
    if isinstance(topics, tuple):
        for topic, prob in zip(*topics):
            ...
            print '{0} ({1}): {2}'.format(topic, float(prob),
                    ', '.join(voc[idx[:n_words]]))
    elif counts is not None:
        for topic, count in zip(topics, counts):
            idx = np.argsort(tau[topic,:].A1)[::-1]
            print '{0} ({1}): {2}'.format(topic, count,
                    ', '.join(voc[idx[:n_words]]))
    else:
        for topic in topics:
            idx = np.argsort(tau[topic,:].A1)[::-1]
            print '{0}: {1}'.format(topic, ', '.join(voc[idx[:n_words]]))
...
if __name__ == '__main__':
    ...
    N, M = X.shape
    print 'Data loaded: {0} documents, {1} words in vocabulary'.format(N, M)
